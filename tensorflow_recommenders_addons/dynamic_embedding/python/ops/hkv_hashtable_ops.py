...
class HkvHashTable(LookupInterface):
  ...
  def __init__(
    ...
    if checkpoint:
      ...
      if not context.executing_eagerly():
        ...
        self.saveable = HkvHashTable._Saveable(self,
                                                  name=name,
                                                  full_name=name)
  ...
  def size(self, name=None):
    ...
    with ops.name_scope(name, "%s_Size" % self.name, [self.resource_handle]):
      with ops.colocate_with(self.resource_handle):
        return hkv_ops.tfra_hkv_hash_table_size(
  ...
  def clear(self, name=None):
    ...
    with ops.name_scope(name, "%s_lookup_table_clear" % self.name,
      ...
      op = hkv_ops.tfra_hkv_hash_table_clear(
          self.resource_handle,
          key_dtype=self._key_dtype,
          value_dtype=self._value_dtype)
  ...
  def insert(self, keys, values, name=None, shared_name=None):
    ...
    with ops.name_scope(
      ...
      with ops.colocate_with(self.resource_handle, ignore_existing=True):
        # pylint: disable=protected-access
        op = hkv_ops.tfra_hkv_hash_table_insert(self.resource_handle,
                                                      keys, values)
    return op
