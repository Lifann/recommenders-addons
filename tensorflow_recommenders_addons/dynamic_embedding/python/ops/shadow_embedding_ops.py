...
class ShadowVariable(de.TrainableWrapper):
  ...
  def _reset_ids(self, ids):
    if not context.executing_eagerly():
      collected_ops = []
    with ops.control_dependencies([self.ids.assign(ids, use_locking=True)]):
      prefetch_op = self.prefetch_values(update=True)
      if not context.executing_eagerly():
        collected_ops.append(prefetch_op)
    for s in self._tracked_slots:
      partial_reset_op = s._reset_ids(ids)
      if not context.executing_eagerly():
        collected_ops.append(partial_reset_op)
    if not context.executing_eagerly():
      return control_flow_ops.group(collected_ops)
