...
class HkvHashtableTest(test.TestCase):
  ...
  #     table = de.get_variable('basic',
  #                             key_dtype=dtypes.int64,
  #                             value_dtype=dtypes.int32,
  #                             initializer=0,
  #                             dim=8,
  #                             init_size=1024,
  #                             kv_creator=de.HkvHashTableCreator(config=de.HkvHashTableConfig(max_capacity=99999)))
  ...
  # #       base_keys = []
  # #       for i in range(18):
  # #         print("i: {}, dim: {}".format(i, dim))
  # #         keys = constant_op.constant(
  # #             np.array([x for x in range(85 * i, 85 * (i+1))]).astype(_type_converter(key_dtype)),
  # #             key_dtype)
  # #         values = constant_op.constant(
  # #             _convert([[x] * dim for x in range(85 * i, 85 * (i+1))], value_dtype),
  # #             value_dtype)

  # #         self.evaluate(table.upsert(keys, values))
  # #         print("should size: {}, {}, {}".format(i, (i+1) * 85, self.evaluate(table.size())))
  # #         self.assertAllEqual((i+1) * 85, self.evaluate(table.size()))
  # #       print("before clear: {}".format(self.evaluate(table.size())))
  # #       self.evaluate(table.clear())
  # #       print("after clear: {}".format(self.evaluate(table.size())))
  # #       del table
    ...
    for (key_dtype, value_dtype), dim, bit in itertools.product(kv_list, dim_list, bits_list):
      ...
      with self.session(config=default_config, use_gpu=True) as sess:
        hkv_table_config = de.HkvHashTableConfig(init_capacity=100000,
                                                 max_capacity=100000,
                                                 max_hbm_for_vectors= bit * (dim + 8) * 100000)
        table = de.get_variable(
            't1-' + str(id) + '_test_variable',
            key_dtype=key_dtype,
            value_dtype=value_dtype,
        ...
        table.clear()
        keys = constant_op.constant(
            np.array([x for x in range(key_num)]).astype(_type_converter(key_dtype)),
            key_dtype)
        values = constant_op.constant(
            _convert([[x] * dim for x in range(key_num)], value_dtype), value_dtype)
