...
def get_sequential_model(emb_t, *args, **kwargs):
  ...
  elif emb_t == de.keras.layers.SlotEmbedding:
...
class EmbeddingLayerTest(test.TestCase):
  ...
  def test_forward(self):
    ...
    de_layer = de.keras.layers.Embedding(4, initializer=init, name='ve820')
    ...
    with self.session(use_gpu=test_util.is_gpu_available(),
      ...
      expected_value = self.evaluate(tf.reduce_sum(tf_layer(ids), 1))
...
@test_util.run_all_in_graph_and_eager_modes
class UnshapedEmbeddingLayerTest(test.TestCase):

  def test_forward(self):
    if not context.executing_eagerly():
      self.skipTest('Only test in eager mode')
    init = tf.keras.initializers.Zeros()
    embedding = de.keras.layers.UnshapedEmbedding(2,
                                                  initializer=init,
                                                  bp_v2=True,
    ...
    preset_ids = constant_op.constant([-1, 0, 1], dtype=dtypes.int32)
    preset_values = constant_op.constant([[1, 1], [1, 1], [1, 1]],
                                         dtype=dtypes.float32)
    embedding.params.upsert(preset_ids, preset_values)
    ids = constant_op.constant([-1, 0, 1, 2], dtype=tf.int32)
    output = de.embedding_lookup(embedding.params, ids, name='vh023')
    expected = embedding(ids)
    self.assertAllEqual(output, expected)


@test_util.run_all_in_graph_and_eager_modes
class SlotEmbeddingLayerTest(test.TestCase):
  ...
  def test_create(self):
    ...
    with self.session(use_gpu=test_util.is_gpu_available(),
      ...
      for comb in itertools.product(dims, key_dtypes, value_dtypes,
                                    ...
                                    restrict_policies):
        name = 'test_creation' + str(rnd)
        embedding = de.keras.layers.SlotEmbedding(comb[0],
                                                  nslots,
                                                  slot_map_fn,
                                                  key_dtype=comb[1],
                                                  value_dtype=comb[2],
  ...
  def test_forward(self):
    ...
    de_layer = de.keras.layers.SlotEmbedding(4,
                                             2,
                                             slot_map_fn,
                                             initializer=init,
                                             name='fr010')
  ...
  def test_backward(self):
    ...
    init = tf.keras.initializers.RandomNormal(seed=0)
    model = get_sequential_model(de.keras.layers.SlotEmbedding,
                                 4,
                                 2,
                                 slot_map_fn,
                                 bp_v2=True,
  ...
  def test_sequential_model_save_and_load_weights(self):
    ...
    new_model = get_sequential_model(de.keras.layers.SlotEmbedding,
                                     4,
                                     2,
                                     slot_map_fn,
                                     bp_v2=False,
    ...
    class MyModel(tf.keras.Model):
      ...
      def __init__(self):
        ...
        self.l0 = tf.keras.layers.InputLayer(input_shape=(None,),
                                             dtype=tf.int64)
        self.l1 = de.keras.layers.SlotEmbedding(4,
                                                2,
                                                slot_map_fn,
                                                bp_v2=False,
                                                initializer=init,
